...
public class EmployeePayrollDetailsRepository {
	...
	private static int getCurrentIdCursor() {
		String query = "SELECT MAX(record_id) FROM EMPLOYEE_PAYROLL_DETAILS";
	...
	public static void addEmployeePayrollDetails(
		...
		String query = "INSERT INTO EMPLOYEE_PAYROLL_DETAILS VALUES ("
		...
		try {
			...
			while (employeePayrollDetailsInDb.next()) {
				...
				employeePayrollDetailsList.add(new EmployeePayrollDetails(
						employeePayrollDetailsInDb.getInt("id"),
						employeeRecord,
	...
	public static List<EmployeePayrollDetails> getAllEmployeePayrollDetails() {
		return getEmployeePayrollDetailsByQuery("SELECT * FROM EMPLOYEE_PAYROLL_DETAILS ORDER BY ID desc");
	}

	public static EmployeePayrollDetails getAllEmployeePayrollDetailsByEmployeeCode(String employeeCode) {
		List<EmployeePayrollDetails> employeePayrollDetails =getEmployeePayrollDetailsByQuery("SELECT * FROM EMPLOYEE_PAYROLL_DETAILS WHERE EMPLOYEE_CODE='"+employeeCode+"'"); 
	...
	public static void updateEmployeePayrollDetails(EmployeePayrollDetails employeePayrollDetails) {
		String query = "UPDATE EMPLOYEE_PAYROLL_DETAILS SET "
				...
				+ "employment_type='"+ employeePayrollDetails.getEmploymentType() + "' WHERE id="+employeePayrollDetails.getId();
		jdbcTemplate.executeUpdate(query);
