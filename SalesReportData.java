...
public class SalesReportData extends DynamicReportData{
	...
	public JRDataSource createDataSource() {
		DRDataSource dataSource = new DRDataSource("product", "customer", "created", "noOfItems", "salesPrice", "amountPaid", "balance");
		for (SalesRecord salesRecord : salesRecords) {
			dataSource.add( salesRecord.getProduct().getName(), salesRecord.getCustomer().getFirstName()+" "+salesRecord.getCustomer().getLastName()
					, toDate(salesRecord.getCreated()), salesRecord.getNoOfItems(), new BigDecimal(salesRecord.getSalesAmount()), new BigDecimal(salesRecord.getAmountPaid()), new BigDecimal(salesRecord.getBalance()));
		}
		return dataSource;
	}
	...
	public DynamicReport getDynamicReport() {
	    ...
	    column.setPattern("#,###.0");
		report.addColumn(column);
	    column = new DynamicColumn("Amount Paid", "amountPaid", "bigDecimal");
	    column.setPattern("#,###.0");
		report.addColumn(column);
	    column = new DynamicColumn("Balance", "balance", "bigDecimal");
	    column.setPattern("#,###.0");
		report.addColumn(column);
