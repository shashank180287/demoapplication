...
public class CustomerRecordsRepository {
	...
	public static void removeCustomerRecord(CustomerRecord customerRecord) {
		...
		String query = "DELETE CUSTOMER_RECORD WHERE id="+customerRecord.getCustomerCode();
		jdbcTemplate.executeUpdate(query);	
	}
	...
	public static List<CustomerRecord> getCustomerRecordByQuery(String query) {
		...
		try{
			...
			while(customerRecordInDb.next()){
				...
				customerRecord.add(new CustomerRecord(customerRecordInDb.getString("customer_code"),
						customerRecordInDb.getString("first_name"),
						customerRecordInDb.getString("last_name"),
						customerRecordInDb.getInt("mobile_number"),
						customerRecordInDb.getString("email"),
						customerRecordInDb.getString("gender"),
						customerRecordInDb.getString("contact_address"),
						customerRecordInDb.getDate("created"),
						customerRecordInDb.getString("website"),
						customerRecordInDb.getString("description"),
						userLoginDetails));
			}
	...
	public static CustomerRecord getCustomerRecordByCustomerCode(String customerCode) {
		String query = "SELECT * FROM CUSTOMER_RECORD WHERE customer_code ='"+customerCode+"'";
		return getCustomerRecordByQuery(query).get(0);
	}

	public static CustomerRecord getCustomerRecordByByUsername(
			String username) {
		String query = "SELECT * FROM CUSTOMER_RECORD WHERE username ='"+username+"'";
		return getCustomerRecordByQuery(query).get(0);
	}
}
