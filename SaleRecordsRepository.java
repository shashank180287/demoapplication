			...
			if (salesRecordInDb != null) {
				while (salesRecordInDb.next()) {
					...
					salesRecord.add(new SalesRecord(salesRecordInDb
							...
							.getFloat("amount_paid"), salesRecordInDb
							.getFloat("balance"), mode, cusotmer,
...
//			e.printStackTrace();
		...
		salesRecord = Arrays.asList(new SalesRecord("asfgsd", new ProductRecord(), 10, null, null, 12.55f, 13.55f, 10.0f, new TransectionMode(), new CustomerRecord(), new EmployeeRecord(), null, new Date(System.currentTimeMillis())));
		return salesRecord;
	}
...
//				+ employeeCode + "' ORDER BY created desc");
	...
	public static String[][] getCustomerSalesRecord() {
		...
		ResultSet salesRecordInDb = null;
		try {
			salesRecordInDb = jdbcTemplate.executeQuery("select customer_code, sum(sales_amount) from sales_records group by customer_code order by sum(sales_amount) desc");
			if (salesRecordInDb != null) {
				while (salesRecordInDb.next()) {
					String customerCode = salesRecordInDb
							.getString("customer_code");
	...
	public static String[][] getProductSalesRecord() {
		...
		ResultSet salesRecordInDb = null;
		try {
			salesRecordInDb = jdbcTemplate.executeQuery("select product_code, sum(sales_amount) from sales_records group by product_code order by sum(sales_amount) desc");
			if (salesRecordInDb != null) {
				while (salesRecordInDb.next()) {
					String productCode = salesRecordInDb
							.getString("product_code");
	...
	public static Map<String, Double> getProductEquity() {
		Map<String, Double> equities = new HashMap<String, Double>();
		ResultSet productSupplyInDb = null;
		try{
			productSupplyInDb = jdbcTemplate.executeQuery("select product_code, sum(sales_amount) from sales_records group"+
					" by product_code");
